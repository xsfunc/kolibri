...
export const WithdrawPanel = () => {
  const amount = useUnit($withdrawAmount);
  const error = useUnit($withdrawError);
  const pending = useUnit($withdrawPending);
  const onAmountChange = useUnit(withdrawAmountChanged);
  const onMax = useUnit(withdrawMaxClicked);
  const onSubmit = useUnit(withdrawSubmitted);
  const currentUtil = useUnit($currentUtilization);
  const projectedUtil = useUnit($withdrawProjectedUtil);
