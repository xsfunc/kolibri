...
export const RepayPanel = () => {
  ...
  const error = useUnit($repayError);
  const pending = useUnit($repayPending);
  const onAmountChange = useUnit(repayAmountChanged);
  const onMax = useUnit(repayMaxClicked);
  const onSubmit = useUnit(repaySubmitted);
  const currentUtil = useUnit($currentUtilization);
  const projectedUtil = useUnit($repayProjectedUtil);
